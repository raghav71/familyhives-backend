...
module.exports = () => {
    ...
    const getAlbumFile = (id, memoryId, transaction = null) => {
        console.log("AlbumService => getAlbumFile")
        return new Promise(function (resolve, reject) {
            let query = {
                where: { id: id, memory_id: memoryId },
                transaction
            };

            let orm = models.album_files.findOne(query);
            orm.then(resolve).catch(reject);
        })
    }

    const deleteAlbumFile = (id, memoryId, transaction = null) => {
        console.log("AlbumService => deleteAlbumFile")
        return new Promise(function (resolve, reject) {
            let query = {
                where: { id: id, memory_id: memoryId },
                transaction
            ...
            let orm = models.album_files.destroy(query);
            orm.then(resolve).catch(reject);
        })
    }
